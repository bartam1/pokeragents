...
class ToolUsageTracker:
    ...
    _summary: dict[str, dict[str, int]] = field(default_factory=lambda: defaultdict(lambda: defaultdict(int)))
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "calls_by_agent": {
                agent: calls for agent, calls in self._calls.items()
            }
        }
...
class GTODeviationTracker:
    ...
    def get_agent_stats(self, agent_id: str) -> dict:
        ...
        gto_profit = sum(
            self._hand_outcomes.get(h, {}).get(agent_id, 0)
            for h in gto_hands
        )
        deviation_profit = sum(
            self._hand_outcomes.get(h, {}).get(agent_id, 0)
            for h in deviation_hands
        )
        ...
        return {
            ...
            "deviation_avg_profit": deviation_profit / len(deviation_hands) if deviation_hands else 0,
        }
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "summary": {
                ...
                "overall_deviation_rate": total_deviation / (total_gto + total_deviation) if (total_gto + total_deviation) > 0 else 0,
...
def log_tools_used(agent_name: str, result, log_level: str = "info", track: bool = True) -> list[str]:
    """
