...
**Key Features:**
- Opponent stats **injected directly into prompt** (no tool lookup needed)
- Access to pot odds, equity, and position tools
...
|------------|-------|-------|--------|
| **GTO Analyst** | Mathematical/theoretical poker | pot_odds, equity, position | Hand strength, recommended sizing |
...
| `gpt4o/` | Backup folder with knowledge from GPT-4o model runs |
...
poc-pokerbot/
├── backend/
│   ├── main.py                    # CLI entry, experiment runner, results output
│   ├── config.py                  # Settings (env vars, defaults)
│   ├── logging_config.py          # Structured logging, LogCollector
│   ├── exceptions.py              # Custom exceptions
│   └── domain/
│       ├── game/
│       │   ├── models.py          # StructuredGameState, Card, HandResult
│       │   └── environment.py     # PokerKit wrapper, showdown detection
│       ├── player/
│       │   ├── models.py          # PlayerStatistics, KnowledgeBase
│       │   └── tracker.py         # StatisticsTracker (VPIP, PFR, WTSD)
│       ├── agent/
│       │   ├── poker_agent.py     # Simple agent (Agent D)
│       │   ├── ensemble_agent.py  # Multi-agent ensemble (Agent E)
│       │   ├── specialists.py     # GTOAnalyst, ExploitAnalyst, DecisionMaker
│       │   ├── models.py          # ActionDecision, BetSizing (structured output)
│       │   ├── prompts.py         # Shared exploitation guidelines (single source of truth)
│       │   ├── utils.py           # ToolUsageTracker, GTODeviationTracker
│       │   ├── strategies/
│       │   │   └── base.py        # AgentStrategy configs
│       │   └── tools/
│       │       └── basic_tools.py # @function_tool definitions
│       └── tournament/
│           └── orchestrator.py    # TournamentOrchestrator, outcome tracking
├── data/
│   ├── knowledge/                 # Persistent knowledge storage
│   │   ├── calibrated_stats.json  # Shared opponent stats (accumulated across runs)
│   │   ├── agent_d_knowledge.json # Agent D's individual knowledge (per tournament)
│   │   ├── agent_e_knowledge.json # Agent E's individual knowledge (per tournament)
│   │   └── gpt4o/                 # Backup knowledge from GPT-4o runs
│   └── results/                   # Experiment results
│       └── experiment_*.json      # Includes logs + GTO deviation stats
├── docs/
│   ├── ARCHITECTURE.md            # This file - technical architecture
│   ├── EXPERIMENT.md              # Goals, hypotheses, challenges
│   ├── PROJECT_STRUCTURE.md       # Team delegation, struct definitions
...
| `get_position_info(seat, button, players)` | Position classification and strategic advice |
...
**Note**: Opponent statistics are injected directly into prompts - no tools needed for stat lookup.
