...
from backend.domain.agent.specialists import GTOAnalyst, ExploitAnalyst, DecisionMaker
...
class EnsemblePokerAgent:
    ...
    async def decide(self, game_state: StructuredGameState) -> ActionDecision:
        ...
        exploit_task = self._exploit_analyst.analyze(
            state_prompt, opponent_stats, hand_history
        )
    ...
    def _build_state_prompt(self, state: StructuredGameState) -> str:
        ...
        hole_cards_str = " ".join(
            f"{c.rank}{c.suit}" for c in our_player.hole_cards
        ) if our_player.hole_cards else "Unknown"
