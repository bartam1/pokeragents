...
class BetSizing(BaseModel):
    ...
    absolute: float | None = Field(default=None, description="Exact chip amount (e.g., 150)")
    bb_multiple: float | None = Field(
        default=None, description="Big blind multiplier (e.g., 3.0 means 3x BB)"
    )
    pot_fraction: float | None = Field(
        default=None, description="Fraction of pot (e.g., 0.75 means 75% pot)"
    )
